...
function toggle_reservation_time(id, week, day, time, from)
{
	if(session_user_is_admin == '1')
	{
		...
		{
			notify('You are reserving back in time. You can do that because you\'re an admin', 4);  // TODO translate
		}
		else if(week > global_week_number + global_weeks_forward)
		{
			notify('You are reserving more than '+global_weeks_forward+' weeks forward in time. You can do that because you\'re an admin', 4);   // TODO translate
		}
	}

	var user_name = $(id).html();

	if(user_name == '') // TODO Rebuild, we have to set maximum amount of users to reserve one lesson
	{
    if (confirm("Are you sure?") == true) // TODO translate
    {
  		$(id).html('Wait...'); 
	...
	else
	{
		if(offclick_event == 'mouseup' || from == 'details')
		{
			if(user_name == 'Wait...')
			{
				notify('One click is enough', 4); // TODO Either remove this functionality or translate
			}
			else if(user_name == session_user_name || session_user_is_admin == '1')
			{
				if(user_name != session_user_name && session_user_is_admin == '1')
				{
					var delete_confirm = confirm('This is not your reservation, but because you\'re an admin you can remove other users\' reservations. Are you sure you want to do this?'); // TODO Translate
				}
				else
				{
